...
#
# Usage (from host with KUBECONFIG set):
#   scripts/run-sriov-nic-ids-on-node.sh [interface]
#   (default interface: eno2np1). Uses base64 so it works with oc debug (no stdin).
set -euo pipefail
N="${1:-eno2np1}"
SYS="/sys/class/net/$N"
