...
#
# Usage:
#   On the node (e.g. SSH or chroot /host):  scripts/verify-sriov-vfs-eno2np1.sh [interface]
#   From oc debug pod (use host sysfs):      scripts/verify-sriov-vfs-eno2np1.sh /host [interface]
#
# Exit 0 if VFs are present (sriov_numvfs > 0 and virtfn0 exists), non-zero otherwise.
set -euo pipefail
